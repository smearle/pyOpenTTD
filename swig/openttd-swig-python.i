...
////////////////////////////////////////////////////////////////////////
// Headers inserted into openttd_wrap.cpp, 
// from openttd-swig-python.i
...
// This callback hook glues Python into OpenTTD's language 
// independent callback mechanism.
//
// Arguments:
//
// openttd: the C++ OpenTTD object
//
// data: a void pointer which came from openttd->data,
  ...
  va_list arglist)
{
  ...
  // We will pass a PyObject wrapper of openttd, 
  ...
  printf(
    "PythonCallbackHook: openttd 0x%X data 0x%X name \"%s\" params \"%s\"\n", 
    (int)openttd, 
    (int)data, 
    name,
  ...
  // Get the SWIG PyObject *openttdObj wrapper from
  // the userData of the openttd. If the userData is NULL,
  // then somebody forgot to set the it, so we do nothing. 
  if (openttd->userData == NULL) {
    // We have not been hooked up yet, so do nothing.
    return;
  }

  PyObject *openttdObj = 
    (PyObject *)openttd->userData;

  // Need to increment the reference count here.
  Py_INCREF(openttdObj);

  int i = 0; // Parameter index.
  
  // Pass the SWIG openttd wrapper as the first parameter. 
  PyTuple_SetItem(
    paramTuple,
    i,
    openttdObj);
...
%extend openttd {
    ...
    // but then use "self" to refer to the openttd object. 
